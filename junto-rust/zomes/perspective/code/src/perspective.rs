...
pub fn create_perspective(name: String) -> ZomeApiResult<EntryAndAddress<app_definition::Perspective>>{
    ...
    let perspective_entry = app_definition::Perspective{name: name, parent: current_agent_username.address.clone()};
    let perspective_address = hdk::api::commit_entry(&Entry::App("perspective".into(), perspective_entry.clone().into()))?;
    hdk::api::link_entries(&current_agent_username.address, &perspective_address, "perspective", "")?;
...
pub fn add_user_to_perspective(perspective: Address, target_user: Address) -> ZomeApiResult<Address>{
    ...
    let current_agent_username = utils::helpers::call_and_get_current_user_username()?;
    hdk::api::link_entries(&target_user, &current_agent_username.address, "follower", "")?;
    hdk::api::link_entries(&current_agent_username.address, &target_user, "following", "")?;
...
pub fn get_perspectives_users(perspective: Address) -> ZomeApiResult<Vec<EntryAndAddress<app_definition::UserName>>> {
    ...
    let current_agent_username = utils::helpers::call_and_get_current_user_username()?;
    if perspective_entry.parent == current_agent_username.address{
