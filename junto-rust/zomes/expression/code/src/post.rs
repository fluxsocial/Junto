...
pub fn post_comment_expression(expression: app_definition::ExpressionPost, parent_expression: Address) -> ZomeApiResult<Address> {
    ...
    let indexes = vec![hashmap!{"type" => "channel".to_string(), "value" => "*null*".to_string()},
                            ...
                            hashmap!{"type" => "channel".to_string(), "value" => "*null*".to_string()},
                            hashmap!{"type" => "channel".to_string(), "value" => "*null*".to_string()},
                            hashmap!{"type" => "user".to_string(), "value" => current_user_username.entry.username.to_lowercase()},
...
pub fn build_hooks<'a>(contexts: Vec<Address>, address: &Address, indexes: &'a Vec<HashMap<&'static str, String>>, index_string: &'a str) 
    ...
    let users_pack = hdk::call(hdk::THIS_INSTANCE, "group", Address::from(hdk::PUBLIC_TOKEN.to_string()), 
                                "get_user_pack", JsonString::from(json!({"username_address": current_user_username.address})))?;
    let users_pack: EntryAndAddress<app_definition::Group> = users_pack.try_into()?;

    let member_results = hdk::call(hdk::THIS_INSTANCE, "user", Address::from(hdk::PUBLIC_TOKEN.to_string()), 
                                "get_user_member_packs", JsonString::from(json!({"username_address": current_user_username.address})))?;
    let EntryAndAddressVec(member_results): EntryAndAddressVec<app_definition::Group> = member_results.try_into()?;
    ...
    let agents_dens = hdk::call(hdk::THIS_INSTANCE, "collection", Address::from(hdk::PUBLIC_TOKEN.to_string()), 
                                "get_user_dens", JsonString::from(json!({"username_address": current_user_username.address})))?;
    let agents_dens: UserDens = agents_dens.try_into()?;
    ...
    for context in &contexts{
        if context == &dna_hash_string{
            ...
            hook_definitions.push(FunctionDescriptor{name: "link_expression", parameters: FunctionParameters::LinkExpression{link_type: "expression_post", tag: index_string, direction: "forward", parent_expression: current_user_username.address.clone(), child_expression: address.clone()}});
            ...
            hook_definitions.push(FunctionDescriptor{name: "create_post_index", parameters: FunctionParameters::CreatePostIndex{indexes: indexes, context: users_pack.address.clone(), expression: address.clone(), index_string: index_string, link_type: "expression_post"}});
            ...
            let _privacy_auth_result = indexing::run_context_auth(context, &current_user_username.address)?
...
pub fn handle_resonation(expression: Address) -> ZomeApiResult<String>{
    ...
    let hook_definitions = vec![FunctionDescriptor{name: "create_post_index", parameters: FunctionParameters::CreatePostIndex{indexes: &indexes, context: users_pack.address.clone(), expression: expression.clone(), link_type: "resonation", index_string: index_string.as_str()}},
                                FunctionDescriptor{name: "link_expression", parameters: FunctionParameters::LinkExpression{link_type: "resonation", tag: "", direction: "forward", parent_expression: users_pack.address, child_expression: expression.clone()}},
                                FunctionDescriptor{name: "link_expression", parameters: FunctionParameters::LinkExpression{link_type: "resonation", tag: "", direction: "forward", parent_expression: expression, child_expression: current_user_username.address}}];
    utils::helpers::handle_hooks(hook_definitions)?;
