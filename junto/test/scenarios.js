async function registerAgent(t, agent, username, first_name, last_name) {
    const register_result = await agent.call('junto', 'user', 'create_user', {user_data: {username: username, first_name: first_name, last_name: last_name, bio: "Junto Testing", profile_picture: "pictureurl"}});
...
async function postExpression(t, agent, expression, attributes, context) {
    ...
    const post_expression = await agent.call('junto', 'expression', 'post_expression', {expression: expression, attributes: attributes, context: context});
...
async function postComment(t, agent, expression, parent_expression) {
    const comment_expression = await agent.call('junto', 'expression', 'post_comment_expression', {expression: expression, parent_expression: parent_expression});
...
async function updateBitPrefix(t, agent, bit_prefix) {
    const update_bit_prefix_value = await agent.call('junto', 'config', 'update_bit_prefix', {bit_prefix: bit_prefix});
...
async function resonation(t, agent, expression) {
    const resonate = await agent.call('junto', 'expression', 'post_resonation', {expression: expression});
...
async function getExpression(t, agent, address) {
    const get = await agent.call('junto', 'expression', 'get_expression', {expression: address});
...
async function addUserToPerspective(t, agent, perspective, target_user){
    const add_user_to_perspective = await agent.call('junto', 'perspective', 'add_user_to_perspective', {perspective: perspective, target_user: target_user});
...
async function isCollectionOwner(t, agent, den, user){
    ...
    const owner_status = await agent.call('junto', 'collection', 'is_collection_owner', {collection: den, username_address: user});
...
async function isGroupMember(t, agent, group, user) {
    const is_group_member = await agent.call('junto', 'group', 'is_group_member', {group: group, username_address: user});
...
async function getGroupMembers(t, agent, group, expect_ok) {
    const get_group_members = await agent.call('junto', 'group', 'get_group_members', {group: group});
...
async function addPackMember(t, agent, user) {
    const add_group_member = await agent.call('junto', 'group', 'add_pack_member', {username_address: user});
...
async function getUserPack(t, agent, user) {
    const get_pack = await agent.call('junto', 'group', 'get_user_pack', {username_address: user});
...
async function removeGroupMember(t, agent, user, group) {
    const remove_group_member = await agent.call('junto', 'group', 'remove_group_member', {username_address: user, group: group});
...
async function getHolochainEnv(t, agent) {
    const holochain_env = await agent.call('junto', 'config', 'get_env', {});
...
async function getDens(t, agent, user) {
    const get_dens = await agent.call('junto', 'collection', 'get_user_dens', {username_address: user});
...
async function getPerspectivesUsers(t, agent, perspective) {
    const perspective_users = await agent.call('junto', 'perspective', 'get_perspectives_users', {perspective: perspective});
